...
import { 
  UserPlus, Lock, Loader2, RefreshCw, Eye, EyeOff, Trash2, AlertTriangle
} from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        {currentUserRole !== 'Etat-Major' && (
          <Alert variant="destructive" className="bg-red-500/10 border-red-500/20 text-red-400">
            <AlertTriangle className="h-4 w-4" /><AlertTitle>Accès Restreint</AlertTitle><AlertDescription>Seul l'État-Major peut gérer les comptes.</AlertDescription>
          </Alert>
        )}
        ...
        <Card className="border-white/10 bg-card/30">
          <Table>
            <TableHeader><TableRow className="hover:bg-transparent border-white/10"><TableHead>Nom</TableHead><TableHead>Rôle</TableHead><TableHead>Mot de passe</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
            <TableBody>
              {loading ? <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">Chargement...</TableCell></TableRow> : profiles.map((p) => (
